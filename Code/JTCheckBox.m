...
#import "JTPathManager.h"

@interface JTCheckBox ()

@property (nonatomic,strong) CAShapeLayer *onBoxLayer;
@property (nonatomic,strong) CAShapeLayer *offBoxLayer;
@property (nonatomic,strong) CAShapeLayer *checkMarkLayer;
@property (nonatomic,strong) JTAnimationManager *animationManager;
@property (nonatomic,strong) JTPathManager *pathManager;

@end

@implementation JTCheckBox
...
- (instancetype)initWithFrame:(CGRect)frame JTBoxType:(BoxType)BoxType JTAnimationType:(AnimationType)AnimationType onTintColor:(UIColor *)OTcolor TintColor:(UIColor *)Tcolor lineWidth:(CGFloat)width animationDuration:(CGFloat)Duration minimumTouchSize:(CGSize)touchSize
{
    ...
    if (self) [self commonInitWithJTBoxType:BoxType JTAnimationType:AnimationType onTintColor:OTcolor TintColor:Tcolor lineWidth:width animationDuration:Duration minimumTouchSize:touchSize];
    return self;
}
...
- (void)commonInitWithJTBoxType:(BoxType)BoxType JTAnimationType:(AnimationType)AnimationType onTintColor:(UIColor *)OTcolor TintColor:(UIColor *)Tcolor lineWidth:(CGFloat)width animationDuration:(CGFloat)Duration minimumTouchSize:(CGSize)touchSize
{
    _on = NO;
    _hideBox = NO;
    _onTintColor = OTcolor;
    _onFillColor = [UIColor clearColor];
    _onCheckColor = OTcolor;
    _tintColor = Tcolor;
    _lineWidth = width;
    _animationDuration = Duration;
    _minimumTouchSize = touchSize;
    _onAnimationType = AnimationType;
    _offAnimationType = AnimationType;
    _boxType = BoxType;
...
- (void)initPathManager {
    _pathManager = [JTPathManager new];
    _pathManager.lineWidth = _lineWidth;
    _pathManager.boxType = _boxType;
}

- (void)initAnimationManager {
    _animationManager = [[JTAnimationManager alloc] initWithAnimationDuration:_animationDuration];
}
...
- (void)setOffAnimationType:(AnimationType)offAnimationType {
...
- (void)addOnAnimation {
    ...
    switch (self.onAnimationType) {
        ...
        case AnimationTypeBounce: {
            CGFloat amplitude = (self.boxType == BoxTypeSquare) ? 0.20 : 0.35;
